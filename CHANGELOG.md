...
- `docs/handoff-features-v2.md` — second-round feature backlog (shared F1–F4 across the three sibling repos: JSON import, serving-size selector, PNG-export i18n fix, shareable `?r=` recipe URLs; plus i18n/a11y completion and e2e gap-fill). `docs/ALIGNMENT.md` updated in step: v0's missing JSON export recorded as a verified parity exception, new "Features v2" section, follow-ups refreshed.
- `docs/ALIGNMENT.md` — the cross-repo alignment overview (shared standards, feature-parity matrix, follow-ups) previously only in the uncommitted parent folder; committed here in the reference implementation so it is version-controlled.

- **Broader e2e coverage** — new Playwright specs exercise the previously untested user flows: `e2e/language.spec.ts` (switching the UI language updates the visible text and `html[lang]`, and the choice persists across reload), `e2e/theme.spec.ts` (dark/light toggle updates the Tailwind `dark` class and persists across reload), `e2e/recipe-history.spec.ts` (save → appears in panel → favorite → both persist across reload → delete → stays gone after reload), and `e2e/custom-proportions.spec.ts` (toggle custom mode → edit a proportion in the `ProportionEditor` → main amounts recalculate → reset restores defaults). `e2e/pwa.spec.ts` replaces the old skipped manifest test with real coverage of the update prompt: it renders on an update signal, Reload invokes the update action, and Dismiss hides it. To make the SW-driven prompt testable under `vite dev`, `PwaUpdatePrompt` gained a minimal, production-inert testability seam — a `pwa:need-refresh` window-event listener that forces the prompt open, and an optional `window.__pwaReloadForTest` hook so a test can observe the Reload action (production never dispatches the event nor sets the hook). Supporting `data-testid` attributes were added across `ThemeLanguageSelector`, `ProportionEditor`, `RecipeHistoryPanel`, and `PwaUpdatePrompt` (no behavior change).
...
- `docs/ALIGNMENT.md` refreshed after the 2026-07-18 tooling-parity pass (bolt #32, v0 #28, Scaler #31): shared standards now record e2e/coverage-in-CI/bundle-budget/CSP/docs parity across all three repos; completed follow-ups struck through; blocked dependabot majors listed
